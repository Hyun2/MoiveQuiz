...
const App: () => React$Node = () => {
  ...
  const onPress = React.useCallback(() => {
    console.log(mode);
    if (mode === 'answer') {
      setQuizList(quizList.filter((quiz, index) => index !== 0));
    }
  ...
  return (
    <>
      <Container>
        <Contents>
          {quizList.length ? (<Quiz>{mode === 'quiz' ? getInitials(quizList[0]) : quizList[0]}</Quiz>) : <Quiz>퀴즈 끝</Quiz>}
        </Contents>
        {quizList.length ? <Button>
          <Label onPress={onPress}>{mode === 'quiz' ? "확인" : '다음'}</Label>
        </Button> : <Button onPress={retry}><Label>퀴즈 다시 시작</Label></Button>}
      </Container>
    </>
  );
};
